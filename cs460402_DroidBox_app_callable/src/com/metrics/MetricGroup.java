...
public class MetricGroup {
	...
	public Metric getMetricsByID(String metricID) {
		...
		for (Metric m1 : metrics) {
			if (m1.getID().equals(metricID)) return m1;
		}
	...
	public String toString() {
		...
		for (Metric m1 : metrics) {
			...
			if (dbaseProvider.length() != 0) output = output + "  " + dbaseProvider + "\n";
			output = output + "  algorithm looped " + m1.getAlgorithmLoops() + " times" + "\n";
			if (m1.getAlgorithmLoops() != 0) output = output + "    average milliseconds per loop: " + (m1.time/m1.getAlgorithmLoops()) + "\n";
			if (m1.getAlgorithmLoops() != 0) output = output + "    average nanoseconds per loop: " + (m1.nanoTime/m1.getAlgorithmLoops()) + "\n";
			output = output + "  algorithm loaded " + m1.getLoadedNodes() + " Nodes" + "\n";
			output = output + "  database calls: " + m1.getDatabaseCalls().size() + "\n";
			if (m1.getDatabaseCalls().size() != 0) {
				output = output + "    total database call time: " + m1.totalDatabaseCallTime() + " milliseconds, " + m1.totalDatabaseCallNanoTime() + " nanoseconds" + "\n";
				output = output + "    median database call time: " + m1.medianDatabaseCallTime() + " milliseconds, " + m1.medianDatabaseNanoCallTime() + " nanoseconds" + "\n";
				output = output + "    average database call time: " + m1.averageDatabaseCallTime() + " milliseconds, " + m1.averageDatabaseNanoCallTime() + " nanoseconds" + "\n";
				output = output + m1.listDatabaseCalls();
			}
			if (m1.getTime() != 0 && m1.getEndNanoTime() != 0) output = output + "  " + m1.getID() + ": total time took " + m1.time + " milliseconds, " + m1.nanoTime + " nanoseconds" + "\n";
			if (m1.totalDatabaseCallTime() != 0) output = output + "    total datbase time: " + m1.totalDatabaseCallTime() + " milliseconds, total algorithm time: " + (m1.time - m1.totalDatabaseCallTime()) + " milliseconds" + "\n";
			if (m1.totalDatabaseCallNanoTime() != 0) output = output + "    total datbase time: " + m1.totalDatabaseCallNanoTime() + " nanoseconds, total algorithm time: " + (m1.nanoTime - m1.totalDatabaseCallNanoTime()) + " nanoseconds" + "\n";
	...
	public class Metric {
		...
		public String listDatabaseCalls() {
			...
			for (int i = 0; i < databaseCalls.size(); i++) {
				output = output + "    call " + (i+1) + ": " + databaseCalls.get(i).getTime() + " milliseconds, " + databaseCalls.get(i).getNanoTime() + " nanoseconds" + "\n";
			}
