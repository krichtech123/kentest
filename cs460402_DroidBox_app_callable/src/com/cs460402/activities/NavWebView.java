...
public class NavWebView extends WebView {
	...
	private final double DEFAULT_ZOOM = 150.0d;	// zoom level is set to 150% by default;
	...
	private final Scroller mScroller;	// Scroller is a utility class provided by Android that does scroll animation calculations for you
	...
	//make sure the back key navigates back to the previous web page within the webview
	@Override
	...
	public void scrollWebView(int dx, int dy, int imgWidth, int imgHeight) {
		Log.i("CENTER", "raw - dx: " + dx + ", dy: " + dy + ", imgWidth: " + imgWidth + ", imgHeight: " + imgHeight);
		...
		Log.i("CENTER", "scaled: " + mZoomLevel + "% - dx: " + dx + ", dy: " + dy + ", imgWidth: " + imgWidth + ", imgHeight: " + imgHeight);
		...
		Log.i("CENTER","webView width: " + getWidth() + ", webView height: " + getHeight());
		...
		Log.i("CENTER","scrollX: " + getScrollX() + ", scrollY: " + getScrollY());
	...
	public void setPOI() {
		loadUrl("javascript:setPOI('.poiHandicap', '" + AppPrefs.getPOIcolor_handicap(mContext) + "', '" + AppPrefs.getPOIdisplay_handicap(mContext) + "')");
		loadUrl("javascript:setPOI('.poiElevator', '" + AppPrefs.getPOIcolor_elevator(mContext) + "', '" + AppPrefs.getPOIdisplay_elevator(mContext) + "')");
		loadUrl("javascript:setPOI('.poiReception', '" + AppPrefs.getPOIcolor_reception(mContext) + "', '" + AppPrefs.getPOIdisplay_reception(mContext) + "')");
		loadUrl("javascript:setPOI('.poiExit', '" + AppPrefs.getPOIcolor_exit(mContext) + "', '" + AppPrefs.getPOIdisplay_exit(mContext) + "')");
		loadUrl("javascript:setPOI('.poiInfo', '" + AppPrefs.getPOIcolor_info(mContext) + "', '" + AppPrefs.getPOIdisplay_info(mContext) + "')");
		loadUrl("javascript:setPOI('.poiStairs', '" + AppPrefs.getPOIcolor_stairs(mContext) + "', '" + AppPrefs.getPOIdisplay_stairs(mContext) + "')");
		loadUrl("javascript:setPOI('.poiRestrooms', '" + AppPrefs.getPOIcolor_restroom(mContext) + "', '" + AppPrefs.getPOIdisplay_restroom(mContext) + "')");
	}
	...
	public void updateAllNodeInfoBoxes(String startID, String startLabel, String endID, String endLabel) {
		loadUrl("javascript:updateAllNodeInfoBoxes('" + startID + "','" + startLabel + "','" + endID + "','" + endLabel + "')");
	}
	...
	private class WebViewScroller implements Runnable {
		...
		public void run() {
			...
			while (mScroller.computeScrollOffset() && i < 100000) {
				...
				Message msg = mHandler.obtainMessage(HANDLER_CENTER_WEBVIEW,(int)mX,(int)mY);
				mHandler.sendMessage(msg);
				
				i++;	// as a safeguard against endless loop, thread will terminate if 'i' reaches 100000
			}
			// thread exits
		}
	...
	/*
	 * UI level handler - receives messages from both WebViewScroller and JavaScriptInterface
	 */
	private class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case HANDLER_UPDATE_WEBVIEW:	// message from JavaScriptInterface
        		...
        		//Log.i("HANDLER","text: " + mBundle.getString("stepText") + ", left: " + mBundle.getInt("left") + ", top: " + mBundle.getInt("top") + ", width: " + mBundle.getInt("imgWidth") + ", height: " + mBundle.getInt("imgHeight"));
        		Log.i("HANDLER","text: " + mStepText + ", left: " + mLeft + ", top: " + mTop + ", width: " + mWidth + ", height: " + mHeight);
			...
			case HANDLER_CENTER_WEBVIEW: // message from WebViewScroller
				//Log.i("CENTER", "Scroller - mX: " + msg.arg1 + ", mY: " + msg.arg2);
				scrollTo(msg.arg1,msg.arg2);
				
				break;
			case HANDLER_CLOSE_PROGRESS: // message from JavaScriptInterface (close progress bar)
				mProgress.setVisibility(GONE);
        		mNavWebView.setVisibility(VISIBLE);
			...
			case HANDLER_UPDATE_STARTEND:
        		...
        		if (msg.arg1 == 1) {
					...
					Toast.makeText(mContext, "Start set to: " + nodeID, Toast.LENGTH_SHORT).show(); 
				} else {
					// update end location
					AppPrefs.setEndID(nodeID, mContext);
					AppPrefs.setEndLabel(nodeLabel, mContext);
					Toast.makeText(mContext, "End set to: " + nodeID, Toast.LENGTH_SHORT).show(); 
				}
        		updateAllNodeInfoBoxes(AppPrefs.getStartID(mContext), AppPrefs.getStartLabel(mContext), AppPrefs.getEndID(mContext), AppPrefs.getEndLabel(mContext));
				break;
	...
	/*
	 * Sets up an interface bewteen Java (Android) and JavaScript (webpage)
	 * This class runs on its own thread (Android sets up the thread automatically)
	...
	public class JavaScriptInterface {    
		...
		public void updateWebView(String stepText, int left, int top, int imgWidth, int imgHeight) {
			//Log.i("ANDROID","text: " + stepText + ", left: " + left + ", top: " + top + ", width: " + imgWidth + ", height: " + imgHeight);
			...
			Log.i("ANDROID","text: " + myBundle.getString("stepText") + ", left: " + myBundle.getInt("left") + ", top: " + myBundle.getInt("top") + ", width: " + myBundle.getInt("imgWidth") + ", height: " + myBundle.getInt("imgHeight"));
